...
// Define NavLink interface here instead of importing it
interface NavLink {
  name: string;
  href: string;
}
...
const MobileMenu: React.FC<MobileMenuProps> = ({ 
  ...
  // Scroll to join-beta section
  const scrollToJoinBeta = () => {
    onClose(); // Close menu first
    setTimeout(() => { // Allow time for menu to close before scrolling
      ...
      >
        <div className="flex flex-col h-full overflow-y-auto">
          <div className="flex items-center justify-between px-4 pt-5 pb-2 border-b border-gray-200 dark:border-white/10">
            ...
            <button
              type="button"
              className="p-2 rounded-md text-gray-500 dark:text-white/70 hover:text-gray-700 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/10 transition-all duration-300"
              onClick={onClose}
            >
          ...
          <div className="px-4 py-6 border-t border-gray-200 dark:border-white/10">
            <button onClick={scrollToJoinBeta} className="w-full">
              <AnimatedButton 
                variant="adaptive" 
                size="md"
                animationType="glow"
