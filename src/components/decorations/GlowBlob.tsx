...
const GlowBlob: React.FC<GlowBlobProps> = ({
  ...
  // Convert opacity to the nearest Tailwind opacity class
  const getOpacityClass = (value: number) => {
    const opacityMap: Record<number, string> = {
      0: 'opacity-0',
      5: 'opacity-5',
      10: 'opacity-10',
      15: 'opacity-10', // No opacity-15, using closest
      20: 'opacity-20',
      25: 'opacity-25',
      30: 'opacity-30',
      40: 'opacity-40',
    ...
    // Find the closest opacity value
  ...
  const darkOpacityClass = getOpacityClass(opacity / 2);

  return (
    <div
      className={`absolute ${positionClasses[position]} ${sizeClasses[size]} ${opacityClass} dark:${darkOpacityClass} pointer-events-none z-0 ${className}`}
    >
      <svg
        viewBox="0 0 200 200"
